...
Age_Sero_df <- data_clean |>
  ...
  dplyr::select(Serotype, "EOD", "LOD", "VLOD","Older Children (1-18 years)", "Adults")|>
...
Age_CC_df <- data_clean |>
  ...
  summarise(N = n(), .groups = "drop") |>
  pivot_wider(names_from = Age_cat, values_from = N) |>
  dplyr::select(CC, "EOD", "LOD", "VLOD","Older Children (1-18 years)", "Adults")|>
  replace_na(list("EOD" = 0, "LOD" = 0, "VLOD" = 0, "Older Children (1-18 years)" = 0, "Adults" = 0))|>
  mutate(across(c("EOD", "LOD", "VLOD","Older Children (1-18 years)", "Adults"), ~ paste(., " (", round(. / sum(.) * 100), "%)", sep="")))
...
ST_Age_df <- data_clean |>
  ...
  summarise(N = n(), .groups = "drop") |>
  pivot_wider(names_from = Age_cat, values_from = N) |>
  dplyr::select(CC, ST, "EOD", "LOD", "VLOD","Older Children (1-18 years)", "Adults")|>
  replace_na(list("EOD" = 0, "LOD" = 0, "VLOD" = 0, "Older Children (1-18 years)" = 0, "Adults" = 0))|>
  mutate(across(c("EOD", "LOD", "VLOD","Older Children (1-18 years)", "Adults"), ~ paste(., " (", round(. / sum(.) * 100), "%)", sep="")))|>
...
CC_ST_Total_df <- CC_ST_Age_df |>
  ...
  mutate(Total = sum(c_across(c("EOD", "LOD", "VLOD", "Older Children (1-18 years)", "Adults")), na.rm = TRUE)) |>  # Sum across the dplyr::selected columns
  ungroup()|>
