...
GBS_pleocytosis_WBC_numbers <- read.csv("GBS_pleocytosis_WBC_numbers.csv")

# Join CSF WBC counts
metadata_new <- left_join(metadata, GBS_pleocytosis_WBC_numbers |> rename(ID=Sample) |> select(ID, CSF.WBC))

# CLEAN DATA
# Keep only columns of interest
metadata_clinical <- metadata_new |> 
select(ID, Source, Same_patient, Date.of.cx, DOB, Admitted.to, Penicillin, Vancomycin, Erhthromycin, Clindamycin, WBC, Hb, platelet, ANC, ALC, CSF.WBC)|> 
...
metadata_clinical_Same <- metadata_clinical |> 
  ...
  mutate(isolated_from=ifelse(ID== "73_S73", "blood", isolated_from))
...
metadata_clinical_V5 <- metadata_clinical_V4|>  
  mutate(Neutropenia= ifelse(as.numeric(ANC)*1000 < 1000, 1, 0),
         ...
         CSF_pleiocytosis= ifelse((days_at_dx<= 28 & CSF.WBC>30) | (days_at_dx> 28 & CSF.WBC>=10) , 1, 0))|>
  mutate(CSF_pleiocytosis= ifelse(is.na(CSF_pleiocytosis), 0, CSF_pleiocytosis))|>
  mutate(Meningitis= ifelse((isolated_from == "blood" & CSF_pleiocytosis==0), 0, 1))|>
  mutate(Meningitis= ifelse(ID %in% c("17_S17", "37_S37"), 0, Meningitis))
```
# Add Genomic data 
```{r}
# load data
